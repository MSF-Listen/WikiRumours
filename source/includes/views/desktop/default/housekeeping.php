<?php
	...
	elseif (!$tl->page['error']) {
		...
		$frequencies = $directory_manager->read(SITE_ROOT_DIR . 'housekeeping/autoload', false, true, false);
		foreach ($frequencies as $frequency) {
			$frequency = substr($frequency, strrpos($frequency, '/'));
			$frequency = substr($frequency, strrpos($frequency, '\\') + 1);

			$tasks = $directory_manager->read(SITE_ROOT_DIR . 'housekeeping/autoload/' . $frequency);
			if (count($tasks)) {
